...
public class HttpUserAuthenticationProvider implements AuthenticationProvider {
  ...
  /**
   * Handles the MFA flow for users with MFA enabled.
   *
   * <p>This method determines whether the user needs to: - Set up MFA (first time) - Verify their
   * TOTP code (subsequent logins) - Complete authentication (MFA already verified in this session)
   *
   * @param user the authenticated user
   * @param clientId the client ID
   * @param password the user's password (not used during MFA flow)
   * @return an authentication token or throws exception to trigger redirect
   */
  ...
  /** Custom exception to indicate MFA is required. Contains the redirect URL for the MFA flow. */
