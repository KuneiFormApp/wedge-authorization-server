...
public class OAuth2AuthorizationRevocationLogoutHandler implements LogoutHandler {
  ...
  /**
   * Finds an OAuth2Authorization for the given user ID.
   *
   * <p>Uses the custom findByUserId method available in our authorization service adapters.
   *
   * @param userId the user ID to search for
   * @return the first OAuth2Authorization found for this user, or null if none found
   */
  private OAuth2Authorization findAuthorizationByUserId(String userId) {
    // Use the custom findByUserId method from our adapters
