...
public class LoginController {
  ...
  private final org.springframework.security.core.context.SecurityContextHolderStrategy securityContextHolderStrategy = org.springframework.security.core.context.SecurityContextHolder
  ...
  public String login(
    ...
    if (error == null && logout == null) {
      org.springframework.security.core.Authentication authentication = securityContextHolderStrategy.getContext()
          .getAuthentication();
   ...
   *
   * <p>
   * This page is shown when a user has MFA enabled but hasn't registered their
   * authenticator app
   * yet.
   */
  ...
  /**
   * Regenerates the MFA secret for the current setup session. Clears the existing
   * secret from the
  ...
  public ResponseEntity<byte[]> mfaQrCode(HttpSession session) {
    ...
    try {
      ...
      // Generate QR code
      byte[] qrCodeBytes = generateMfaQrCodeUseCase.execute(
  ...
  /**
   * MFA Setup Verification - verifies the initial TOTP code and completes MFA
   * registration.
   */
  ...
  public String mfaSetupVerify(
    ...
    boolean registered = registerMfaWithProviderUseCase.execute(clientId, user.getUserId(), mfaSecret, mfaKeyId);
   ...
   *
   * <p>
   * Note: The actual authentication completion is handled by the authentication
   * provider. This
  ...
  public String mfaVerifyPost(
    ...
    var savedRequest = (org.springframework.security.web.savedrequest.SavedRequest) session
