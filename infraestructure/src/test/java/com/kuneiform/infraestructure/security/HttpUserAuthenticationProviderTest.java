...
class HttpUserAuthenticationProviderTest {
  ...
  void shouldAuthenticateValidUser() {
    ...
    when(authenticateUserUseCase.execute(TEST_CLIENT_ID, null, "alice", "password123"))
  ...
  void shouldRejectInvalidCredentials() {
    when(authenticateUserUseCase.execute(TEST_CLIENT_ID, null, "alice", "wrongpassword"))
  ...
  void shouldExtractClientIdFromRequestParameter() {
    ...
    when(authenticateUserUseCase.execute("my-client", null, "alice", "password"))
        .thenReturn(Optional.of(user));
    ...
    assertTrue(result.isAuthenticated());
    verify(authenticateUserUseCase).execute("my-client", null, "alice", "password");
  }
  ...
  void shouldFallbackToDefaultClientIdWhenNoClientIdInRequest() {
    ...
    User user = User.builder().userId("user-123").username("alice").build();
    when(authenticateUserUseCase.execute("default-app", null, "alice", "password"))
        .thenReturn(Optional.of(user));
    ...
    assertTrue(result.isAuthenticated());
    verify(authenticateUserUseCase).execute("default-app", null, "alice", "password");
  }
  ...
  void shouldFallbackToDefaultTenantIdWhenNoClientAndNoDefaultClient() {
    ...
    when(authenticateUserUseCase.execute(null, "default-tenant", "alice", "password"))
        .thenReturn(Optional.of(user));
    ...
    assertTrue(result.isAuthenticated());
    verify(authenticateUserUseCase).execute(null, "default-tenant", "alice", "password");
  }
}
