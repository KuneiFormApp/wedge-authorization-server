...
import com.kuneiform.infraestructure.adapter.HttpMfaRegistrationAdapter;
...
class HttpMfaRegistrationAdapterTest {
  ...
  @Test
  void registerMfa_shouldSuccessfullyRegisterWithUserProvider() {
    ...
    when(restClient.registerMfa(anyString(), eq(mfaSecret), eq(mfaKeyId))).thenReturn(true);
  ...
  @Test
  void registerMfa_shouldHandleEndpointTemplateCorrectly() {
    ...
    when(clientRepository.findByClientId(clientId)).thenReturn(Optional.of(client));
    when(tenantRepository.findById(tenantId)).thenReturn(Optional.of(tenant));
    when(restClient.registerMfa(anyString(), anyString(), anyString())).thenReturn(true);
  ...
  @Test
  void registerMfa_shouldReturnFalseWhenRegistrationFails() {
    ...
    when(clientRepository.findByClientId(clientId)).thenReturn(Optional.of(client));
    when(tenantRepository.findById(tenantId)).thenReturn(Optional.of(tenant));
    when(restClient.registerMfa(anyString(), anyString(), anyString()))
        .thenReturn(false); // Registration fails
  ...
  @Test
  void registerMfa_shouldHandleComplexUserIds() {
  ...
  @Test
  void registerMfa_shouldHandleMultipleUserIdPlaceholders() {
