...
public class HttpUserAuthenticationProvider implements AuthenticationProvider {
  ...
  private record ClientContext(String clientId, String tenantId, Set<String> scopes) {}
  ...
  private ClientContext determineClientContext() {
    try {
      ...
      if (clientId != null) {
        return new ClientContext(clientId, null, scopes);
      }
      ...
      if (defaultClientId != null && !defaultClientId.isBlank()) {
        ...
        return new ClientContext(defaultClientId, null, scopes);
      }
      ...
      return new ClientContext(null, defaultTenantId, scopes);
      ...
      String defaultClientId = config.getLogin().getDefaultClientId();
      if (defaultClientId != null && !defaultClientId.isBlank()) {
        return new ClientContext(defaultClientId, null, java.util.Collections.emptySet());
      }
      ...
      return new ClientContext(null, defaultTenantId, java.util.Collections.emptySet());
    }
  }

  private Set<String> extractScopes(HttpServletRequest request) {
    // 1. Try request parameter "scope" or "scopes"
    String scopeParam = request.getParameter("scope");
    if (scopeParam == null) {
      scopeParam = request.getParameter("scopes");
    }

    if (StringUtils.hasText(scopeParam)) {
      return java.util.stream.Stream.of(scopeParam.split("\\s+"))
    ...
    if (session != null) {
      ...
      if (saved instanceof SavedRequest sr) {
        String[] values = sr.getParameterValues("scope");
        if (values != null && values.length > 0 && StringUtils.hasText(values[0])) {
          return java.util.stream.Stream.of(values[0].split("\\s+"))
              .collect(java.util.stream.Collectors.toSet());
        }
      }
    }
